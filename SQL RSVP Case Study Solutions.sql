...
SELECT year, COUNT(id) AS Movies_year_wise
FROM movie
GROUP BY year
ORDER BY year;
...
SELECT Month(date_published) AS Month_Number, COUNT(*) AS Movies_month_wise
FROM movie
...
FROM ratings AS r
INNER JOIN movie AS m ON m.id = r.movie_id limit 10;
...
WITH top_actor AS 
(
 SELECT b.NAME AS actor_name, SUM(c.total_votes) AS total_votes, COUNT(DISTINCT a.movie_id) AS movie_count,
 ...
 WHERE a.category = 'actor' AND d.country LIKE '%India%'
...
WITH top_actress AS 
(
 SELECT b.NAME AS actress_name, SUM(c.total_votes) AS total_votes, COUNT(DISTINCT a.movie_id) AS movie_count,
 ROUND(SUM(c.avg_rating * c.total_votes) / SUM(c.total_votes), 2) AS actress_avg_rating
 FROM role_mapping a
 INNER JOIN names b ON a.name_id = b.id
 INNER JOIN ratings c ON a.movie_id = c.movie_id
 INNER JOIN movie d ON a.movie_id = d.id
 WHERE a.category = 'actress' AND (d.country = 'India') AND (d.languages = 'hindi')
 GROUP BY a.name_id, b.NAME
...
(
 ...
 FROM movie m
 INNER JOIN genre g ON m.id = g.movie_id
 WHERE genre IN(SELECT genre FROM genre_Top3)
)
SELECT *
FROM movie_Top5
WHERE movie_rank<=5;
